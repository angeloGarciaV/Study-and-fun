const router = require('express').Router();
const db = require('../database/');

// Obtener todas las puntuaciones
router.get('/', async (req, res) => {
    try {
        const [results] = await db.query('SELECT * FROM scores');
        res.json(results);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});

// Obtener la puntuación de un usuario específico
router.get('/:user_id', async (req, res) => {
    const { user_id } = req.params;
    try {
        const [results] = await db.query('SELECT * FROM scores WHERE user_id = ?', [user_id]);
        if (results.length === 0) {
            return res.status(404).json({ message: 'Score not found' });
        }
        res.json(results[0]);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});

// Crear o actualizar una puntuación
router.post('/', async (req, res) => {
    const { user_id, score } = req.body;
    try {
        const [existing] = await db.query('SELECT * FROM scores WHERE user_id = ?', [user_id]);
        if (existing.length === 0) {
            const [result] = await db.query('INSERT INTO scores (user_id, score) VALUES (?, ?)', [user_id, score]);
            res.status(201).json({ id: result.insertId, user_id, score });
        } else {
            await db.query('UPDATE scores SET score = ? WHERE user_id = ?', [score, user_id]);
            res.json({ message: "Score updated", user_id, score });
        }
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});

module.exports = router;
